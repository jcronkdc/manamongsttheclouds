#!/usr/bin/env python3
"""
Compiles Part III: The Breaking (Chapters 23-34) into a printable Word document.
"""
...
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Part III - The Breaking.docx")
...
def add_title_page(doc):
    ...
    run = sub.add_run("Part III: The Breaking")
    run.font.size = Pt(16)
    run.font.name = 'Georgia'
    run.italic = True
...
def add_part_header(doc):
    ...
    title.paragraph_format.first_line_indent = Inches(0)
    title.paragraph_format.space_after = Pt(24)
    run = title.add_run("THE BREAKING")
    run.font.size = Pt(22)
    run.font.name = 'Georgia'
    run.bold = True
    ...
    run = ep.add_run('"The world does not ask if you are ready.\nIt asks if you are willing."')
    run.font.size = Pt(11)
    run.font.name = 'Georgia'
    run.italic = True
