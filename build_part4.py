#!/usr/bin/env python3
"""
Compiles Part IV: The Chamber (Chapters 35-42) into a printable Word document.
"""
...
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Part IV - The Chamber.docx")
...
def add_title_page(doc):
    ...
    run = sub.add_run("Part IV: The Chamber")
    run.font.size = Pt(16)
    run.font.name = 'Georgia'
    run.italic = True
...
def add_part_header(doc):
    ...
    run = ep.add_run('"He who takes everything possesses nothing.\nHe who gives everything becomes everything."')
    run.font.size = Pt(11)
    run.font.name = 'Georgia'
    run.italic = True
